...
async def test_post_my_survey_persists_and_returns_submission(client):
...
async def test_put_my_survey_remains_supported_for_idempotent_updates(client):
    current_user = _fake_user()
    app.dependency_overrides[get_current_user] = lambda: current_user
